...
画像为空时，孩子端首屏会**自动引导**认识孩子（姓名 / 年级 / 爱好）；对话写入 M2 后自动进入正常学习阶段，**无需单独填表**。
...
> 当前 catalog 含 **15 单元 / 71 知识点**（数学二年级 5 单元 + 沪教英语三年级 **10 单元** `english-g3-u01`～`u10`）。磁盘上 `kp_catalog.json` 更新后，「浏览知识库」会自动读最新内容，一般无需重启 8770。
...
| 课本听力/看图题出现在知识点入库 | 应在 **习题处理 → 待归类**；若历史 job 残留可运行 `python scripts/migrate_question_jobs_to_inbox.py` |
| 浏览知识库仍显示旧单元 | 刷新页面；或 `POST /api/kp/catalog/reload`；入库脚本跑在 Windows 而 8770 在 WSL 时确认两边指向同一 `kp_catalog.json` |
| 批准入库后 Jarvis 仍不认识新 KP | 再问一题即可（8771 catalog 按文件自动刷新）；若仍无效再重启 8771 |
...
| 现象 | 处理 |
|------|------|
| 等半分钟才有回复 | 正常；界面会轮换「贾维斯在想…」并逐字显示；告诉孩子耐心等 |
...
**给孩子的三句话**：① 贾维斯要想一下才回答，别着急。② 不会的题可以打字、说话，或拍照片。③ 拍完后点橙色按钮，或者说「教教我」。想给助手起别名可以说「以后叫你 XX」。
